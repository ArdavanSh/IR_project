...
def create_and_store_index_and_embeddings():
    ...
    documents = []
    doc_ids = []
    for doc in faculty_collection.find({}, {"text": 1, "_id": 1}):  # Fetch only the 'text' field and '_id'
        documents.append(doc['text'])
        doc_ids.append(str(doc['_id']))  # Use document _id as unique identifier
    ...
    for term_idx, term in enumerate(terms):
        for doc_idx in range(tfidf_matrix.shape[0]):
            ...
            if score > 0:  # Include only terms with non-zero scores
                inverted_index[term].append({"document_id": doc_ids[doc_idx], "tfidf_score": score})

    # Store inverted index in MongoDB
    inverted_index_collection.delete_many({})  # Clear existing collection
    ...
    document_vectors = tfidf_matrix.toarray()  # Convert sparse matrix to dense array
    embeddings_collection.delete_many({})  # Clear existing collection
    for doc_idx, doc_id in enumerate(doc_ids):
        embeddings_collection.insert_one({
            "document_id": doc_id,
            "tfidf": document_vectors[doc_idx].tolist()  # Store vector as a list
        })
...
def exact_search_index(embeddings):
    d = len(next(iter(embeddings.values())))  # Dimension of embeddings
    ...
    index.add(embedding_matrix)  # Add all embeddings
